...
public class Engine 
{	
	public int m_nArrSlot[][] = new int[Resursi.ROW_][Resursi.COL_];
	public int m_nArrTempSlot[][] = new int[Resursi.CHARACTER_COUNT][Resursi.COL_];
	public float m_fBetweenY;
	public float m_fMovingY[] = new float[Resursi.COL_];
	public boolean m_bSloting[] = new boolean[Resursi.COL_];
	public int m_nRuleLineCount;
	public int[] m_nRowIndex = new int[Resursi.COL_];
	...
	public float[] m_fPrevStep = new float[Resursi.COL_];
	public int[] m_fMovingYStep = new int[Resursi.COL_];
	public int[] m_nSlotTick = new int[Resursi.COL_];
	public String[] m_strState = new String[Resursi.COL_];
	...
	public void getInfo(){
		m_nGameCoin = Resursi.allCoin;
		m_nRuleLineCount = Resursi.curLine;
		m_nMaxLineCount = Resursi.maxline;
		m_nBet = Resursi.bet;
	}
	...
	public void initVariable(){
		m_bStartSlot = false;
		getInfo();
		int nPrevStep = Resursi.PREVSTEP;
		for(int i = 0; i < Resursi.COL_ ; i++){
			initCardStartPosY(i);
			m_bSloting[i] = false;
			m_fMovingYStep[i] = Resursi.MIN_VEL;
	...
	public void loadSlot(){
		...
		for(int i = 0; i < Resursi.CHARACTER_COUNT ; i++){
			for(int j = 0; j < Resursi.COL_ ; j++){
				if(i == 0)
					m_nRowIndex[i] = Resursi.rowIndex[i];
				if(i < Resursi.ROW_)
					m_nArrSlot[i][j] = Resursi.arrSlot[i][j];
				m_nArrTempSlot[i][j] = Resursi.arrTempSlot[i][j];
			}
		}
		Resursi.payTableFlag = false;
	}
	public void initSlot(){
		int changeVal= 0;
		if(Resursi.payTableFlag){
			loadSlot();
			return;
		}
		for (int i = 0; i < Resursi.CHARACTER_COUNT; i++) {
		...
		for(int i = 0; i < Resursi.COL_ ; i++){
			for(int j = 0; j < Resursi.CHARACTER_COUNT ; j++){
				boolean bFlag = true;
				while(bFlag){
					changeVal = MathLib.random(0, (Resursi.CHARACTER_COUNT - 1));
					boolean equalState = true;
					for(int k = 0; k < Resursi.CHARACTER_COUNT ; k++){
	...
	public void resetSlot(int nColIndex){
		if(m_fMovingY[nColIndex] >= m_fBetweenY){
			for(int j = Resursi.ROW_ - 2; j >= 0 ; j--){
			...
			if(m_nRowIndex[nColIndex] < 0)
				m_nRowIndex[nColIndex] = Resursi.CHARACTER_COUNT - 1;
			Resursi.playEffect(Resursi.spin);
	...
	public void moveSlot(int nColIndex){
		if(m_strState[nColIndex].equals("prev")){
			...
			if(m_strState[nColIndex].equals("normal") && m_fMovingYStep[nColIndex] < Resursi.MAX_VEL)
				m_fMovingYStep[nColIndex] += 1;
			if(m_strState[nColIndex].equals("last")){
				if(m_fMovingYStep[nColIndex]> Resursi.MIN_VEL){
					m_fMovingYStep[nColIndex] -= 1;
					if(m_fMovingYStep[nColIndex] < Resursi.MIN_VEL)
						m_fMovingYStep[nColIndex] = Resursi.MIN_VEL;
				}
			}		
	...
	public void setState(int tick, int nColIndex){
		...
		if(m_bSloting[nColIndex] && m_strState[nColIndex].equals("last")){
			if(m_nSlotTick[nColIndex] < Resursi.TICK)
	...
	public void compareCards(){
		...
		for(int nRuleLineIndex = 0 ; nRuleLineIndex < m_nRuleLineCount ; nRuleLineIndex++){			
			for(int nEqualIndex = 0; nEqualIndex < Resursi.COL_ - 1 ; nEqualIndex++){
			...
			if(nCoin > 0){
				...
				Resursi.TGameResult.add(r);
				m_nWin += nCoin * m_nBet;				
				Resursi.playEffect(Resursi.seccess);
				nEqualCount = 1;
				nCardType = 1;
			}
		}
		if(m_bHit){
			...
			Resursi.playEffect(Resursi.fire_btn);
			m_nGameCoin -= m_nBet*m_nRuleLineCount;			
		}
		Resursi.allCoin = m_nGameCoin;
		Resursi.saveSetting();
	}
